...
class Thermalmodel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'D_real', 'D_fake', 'G', 'NCE','SGAN']
        self.visual_names = ['real_A', 'idt_A','fake_B', 'real_B','fake_A']
        self.nce_layers = [int(i) for i in self.opt.nce_layers.split(',')]
        
            

        if opt.nce_idt and self.isTrain:
            self.loss_names += ['NCE_Y','idt_B','NCE_YA','idt_A','NCE_YB']
    ...
    def compute_G_loss(self):
        ...
        if self.opt.nce_idt and self.opt.lambda_NCE > 0.0:
            ...
            self.loss_idt_A = self.criterionIdt(self.idt_A, self.real_B)
            self.loss_NCE_YB = self.calculate_NCE_loss(self.real_B, self.idt_B)
            self.loss_NCE_YA = self.calculate_NCE_loss(self.real_A, self.idt_A)
            
            #self.loss_NCE_YFB = self.calculate_NCE_loss(self.real_A, self.fake_B)
            #loss_NCE_both = (self.loss_NCE + self.loss_NCE_Y + self.loss_NCE_YB) * 0.5  #* 0.50
            loss_NCE_both = (self.loss_NCE  + self.loss_NCE_Y ) * 0.5  #* 0.50
            loss_NCE_both = loss_NCE_both + (self.loss_NCE_YA )  + self.loss_NCE_YB * 0.40
            self.loss_G = self.loss_G_GAN + loss_NCE_both  +  (  self.loss_idt_A *0.50)  + 0.30 * self.loss_SGAN #cambio para ver resultado
        else:
            loss_NCE_both = ((self.loss_NCE + self.loss_NCE_Y ) * 0.5) + (self.loss_NCE + self.loss_NCE_Y ) * 0.5

            self.loss_G = self.loss_G_GAN + loss_NCE_both  +   0.45 * self.loss_SGAN # (  self.loss_idt_A *0.50)  #cambio para ver resultado

        #self.loss_G = self.loss_G_GAN + loss_NCE_both  +  ( self.loss_idt_B *0.75 + self.loss_idt_A *0.70)  + 0.30 * self.loss_SGAN #cambio para ver resultado
